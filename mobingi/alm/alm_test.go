...
func TestWalkerDevAcct(t *testing.T) {
	...
	if os.Getenv("MOBINGI_CLIENT_ID") != "" && os.Getenv("MOBINGI_CLIENT_SECRET") != "" {
		...
		in := WalkerCtx{
			Data: data,
			StackCallback: func(data interface{}, body []byte, ls *ListStack) error {
				_data := data.(*data_t)
				if _data.Data != "hello" {
			...
			InstanceCallback: func(data interface{}, body []byte, ls *ListStack, flag string, inst *Instance, err error) error {
